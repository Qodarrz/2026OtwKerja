  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4 sm:p-6 lg:p-8 font-sans">
      <div className="max-w-2xl w-full">
            ...
            {step === 'upload' && (
              <div className="space-y-8">
                <div className="text-center">
                  <h2 className="text-xl font-bold text-gray-900 mb-2">Unggah Foto KTP</h2>
                  <p className="text-sm text-gray-500">Pastikan foto terlihat jelas, tidak blur, dan tidak terpotong.</p>
                </div>

                <div 
                  onClick={() => fileInputRef.current?.click()}
                  className={`relative group cursor-pointer border-2 border-dashed rounded-3xl p-10 transition-all duration-300 flex flex-col items-center justify-center space-y-4 ${preview ? 'border-indigo-600 bg-indigo-50/30' : 'border-gray-200 hover:border-indigo-400 hover:bg-gray-50'}`}
                >
                  <input 
                    type="file" 
                    ref={fileInputRef} 
                  ...
                  {preview ? (
                    ...
                    <>
                      <div className="w-20 h-20 bg-gray-50 rounded-full flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                        <Upload className="w-8 h-8 text-gray-400 group-hover:text-indigo-600 transition-colors" />
                      </div>
                      <div className="text-center">
                        <p className="text-base font-semibold text-gray-900">Klik untuk pilih atau tarik file</p>
                        <p className="text-xs text-gray-400 mt-1">PNG, JPG atau JPEG (Maks. 5MB)</p>
                      </div>
                    </>
                  )}
                </div>
                ...
                <div className="pt-4 space-y-3">
                  <button
                    ...
                    className="w-full h-14 bg-indigo-600 hover:bg-indigo-700 disabled:bg-gray-200 disabled:text-gray-400 text-white font-bold rounded-2xl transition-all flex items-center justify-center gap-3 shadow-lg shadow-indigo-100 active:scale-[0.98]"
                  >
                    {isScanning ? <Loader2 className="w-5 h-5 animate-spin" /> : <>Mulai Verifikasi <ArrowRight className="w-5 h-5" /></>}
                  </button>
                  <button
                    onClick={() => router.push('/submit')}
                    className="w-full h-12 bg-white hover:bg-gray-50 text-gray-500 font-medium rounded-2xl transition-all border border-gray-100 flex items-center justify-center"
                  >
            ...
            {step === 'scanning' && (
              <div className="py-12 space-y-10 text-center">
                <div className="relative w-48 h-48 mx-auto">
                  ...
                  <div className="absolute inset-4 bg-indigo-600/20 rounded-full animate-pulse" />
                  
                  <div className="relative z-10 w-full h-full bg-white rounded-full border-4 border-indigo-50 flex items-center justify-center overflow-hidden">
                    <Scan className="w-20 h-20 text-indigo-600 animate-pulse" />
                    {/* Scanning Line */}
                    <div className="absolute top-0 left-0 w-full h-1 bg-indigo-600 shadow-[0_0_15px_rgba(79,70,229,0.5)] animate-scan" />
                  </div>
                </div>
                
                <div className="space-y-3">
                  <h2 className="text-2xl font-bold text-gray-900">Menganalisa Data...</h2>
                  <p className="text-gray-500">Sistem AI kami sedang mengekstrak informasi dari KTP Anda secara otomatis.</p>
                </div>
              ...
              <div className="space-y-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-gray-500 uppercase tracking-wider flex items-center gap-2">
                      <CreditCard className="w-4 h-4 text-indigo-500" /> NIK
                    </label>
                    <input 
                      type="text"
                      value={extractedData.nik}
                      onChange={(e) => setExtractedData({...extractedData, nik: e.target.value})}
                      className="w-full h-12 bg-gray-50 border border-gray-100 rounded-xl px-4 text-sm font-semibold focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
                    />
                  </div>
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-gray-500 uppercase tracking-wider flex items-center gap-2">
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-gray-500 uppercase tracking-wider flex items-center gap-2">
                      <MapPin className="w-4 h-4 text-indigo-500" /> Tempat Lahir
                    </label>
                    <input 
                      type="text"
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-gray-500 uppercase tracking-wider flex items-center gap-2">
                      <FileText className="w-4 h-4 text-indigo-500" /> Jenis Kelamin
                    </label>
                    <select 
                      value={extractedData.gender}
                      onChange={(e) => setExtractedData({...extractedData, gender: e.target.value})}
                      className="w-full h-12 bg-gray-50 border border-gray-100 rounded-xl px-4 text-sm font-semibold focus:ring-2 focus:ring-indigo-500 outline-none transition-all appearance-none"
                    >
                  ...
                  <div className="sm:col-span-2 space-y-2">
                    <label className="text-xs font-bold text-gray-500 uppercase tracking-wider flex items-center gap-2">
                      <MapPin className="w-4 h-4 text-indigo-500" /> Alamat
                    </label>
                    <textarea 
                      value={extractedData.address}
                      onChange={(e) => setExtractedData({...extractedData, address: e.target.value})}
                      rows={3}
                      className="w-full bg-gray-50 border border-gray-100 rounded-xl p-4 text-sm font-semibold focus:ring-2 focus:ring-indigo-500 outline-none transition-all resize-none"
                    />
                  </div>
                </div>
                ...
                <div className="pt-4 space-y-3">
                  <button
                    ...
                    className="w-full h-14 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-2xl transition-all flex items-center justify-center gap-2 shadow-lg shadow-indigo-100 active:scale-[0.98]"
                  >
                  ...
                  <button
                    ...
                    disabled={isConfirming}
                    className="w-full h-12 bg-white hover:bg-gray-50 text-gray-600 font-semibold rounded-2xl transition-all border border-gray-200"
                  >
                    Foto Ulang
                  </button>
                </div>
        ...
        {/* Security Notice */}
        <p className="mt-8 text-center text-xs text-gray-400 flex items-center justify-center gap-2">
